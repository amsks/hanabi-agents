...
class ParallelRulebasedAgent():
  ...
  def __init__(self, rules):
    ...
    self.diversities = [[0 for _ in range(len(rules))] for _ in range(len(rules))]
    self.num_comparisons = 1

  def next_agent(self):
    self.agent_turn = (self.agent_turn + 1) % self.n_agents

  def update_diversities(self, observation):
    offset = observation.current_player_offset
    for i in range(self.n_agents):
      for j in range(self.n_agents):
        a0 = self.rules[self.agent_turn]
        a1 = self.rules[offset]
        cpi = self.agent_turn
        cpo = observation.current_player_offset
        self.diversities[cpi][cpo] = (self.diversities[cpi][cpo] + (int(not(a0 == a1)))) / self.num_comparisons
        offset = (self.agent_turn + 1 + offset) % self.n_agents
        self.next_agent()
    self.num_comparisons += 1
